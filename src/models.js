...
export async function initLabel() {
  ...
  for (const existingLabel of labels) {
    ...
    if (existingLabel.name === utils.defaultLabel.name)
      defaultLabel = existingLabel;
  }
...
export async function update(issue) {
  ...
  const commentText = utils.getCommentText(
...
export async function unblockPRs(issueNumber) {
  ...
  for (const pr of blockedPRs) {
    ...
    if (github.isPR(pr))
      await github.rerunAction(pr.number); // only works on prs
